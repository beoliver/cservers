...
int main(int argc, const char* argv[]) {
    ...
    char* user_fd_name[MAX_USERS];
    int uname_waiting[MAX_USERS];
    ...
    ascii_trie* user_name_fd = ascii_trie_init();
...
//                    connection on some existing client socket
                    ...
                    if ((nbytes = read_from_connected_client(fd, buffer)) < 1) {
                        ...
                        fflush(stderr);
                        close(fd);
                        user_fd_name[fd] = NULL;
                        FD_CLR(fd, &active_fd_set);
                    } else {
                        
                        if (user_fd_name[fd] == NULL) {
                            char* uname = "please select usersname...\n";
                            uname_waiting[fd] = 1;
                            send(fd, uname, strlen(uname), 0);
                        }
